...
<div>
  ...
  <div id="table-scroll" class="table-scroll" ref="TableScroll">
       ...
       <table class="main-table">
         <tbody ref="TableScrollTBody">
          <tr v-for="(row, ir) in rows" :class="row.class" :key="ir">
            ...
            <td v-for="(col, ic) in cols" v-if="ic>0" v-html="row[ col.key ]" :key="ic">
            </td>
          </tr>
         </tbody>
       </table>
    ...
    <template v-for="(row, ir) in rows" v-if="row.class=='scrollrow'">
    <div  class="SecondScroll SecondScrollAdditional" @scroll="OnScrollSecond" :ref="'SecondScroll'+ir" :key="'SecondScroll'+ir">
      <div class="SecondScrollInner" :ref="'SecondScrollInner'+ir"></div>
    </div>
    </template>
...
//console.log(this.$refs.SecondScrollInner.offsetHeight)
...
//this.$refs['SecondScrollInner'+i][0].style.height = h+'px';

            this.$refs[('SecondScroll'+i)][0].style.width = w+'px';

            let top = this.$refs['tr'+i][0].offsetTop - 2;
            this.$refs[('SecondScroll'+i)][0].style.top = top+'px';
            //console.log(this.$refs.SecondScrollInner);
                

  //          let h = this.$refs['tr'+i][0].offsetTop - 1;
            //this.$refs[('SecondScroll'+i)][0].style.height = '33px';
